...
from utils.utils import save_checkpoint, AverageMeter, Logger, accuracy, mkdirs, adjust_learning_rate, time_to_str, to_categorical_ml, to_categorical
from utils.evaluate import eval4 as eval
from utils.get_loader import get_dataset
from models.DGFAS_netvlad import DG_model_vlad, Discriminator_share_grl
...
def train():
    ...
    num_cluster, dim, alpha, stride, vlad_alpha, adap, adap_num = 16, 128, 3., 1, False, True, 0.1, -1
    total_dim = dim * num_cluster
    print(num_cluster, dim, alpha, stride, vlad_alpha, adap, 'DG_model, 1, 1, vlad_cent, 8*8')
    net = DG_model_vlad(config.model, num_cluster=num_cluster, dim=dim, alpha=alpha, adap=adap).to(device)
    # ad_net_real = Discriminator_share(dim=total_dim, coeff=-1.).to(device)
    ad_net_real = Discriminator_share_grl(dim=total_dim).to(device)
    ...
    for iter_num in range(max_iter+1):
        ...
        ######### forward #########
        classifier_label_out, feature, soft_assign, local_ = net(input_data, config.norm_flag)
        ...
        discriminator_out = ad_net_real(feature)
        ...
        ad_label = torch.cat([ad_label1, ad_label1_fake, ad_label2, ad_label2_fake, ad_label3, ad_label3_fake],
                             dim=0).view(-1)
        ad_loss = criterion["softmax_ad"](discriminator_out, ad_label)
        cls_loss = criterion["softmax"](classifier_label_out.narrow(0, 0, input_data.size(0)), source_label)
        v_adapt_loss = 0
        if adap == True and config.lambda_v_adapt > 0 and iter_num > adap_num:
        ...
        ######### backward #########
        total_loss = cls_loss + config.lambda_triplet * triplet + config.lambda_adreal * ad_loss + v_adapt_loss * config.lambda_v_adapt
        ...
        source_label_domain = ad_label
        acc_domain = accuracy(discriminator_out, source_label_domain, topk=(1,))
        classifer_top1_domain.update(acc_domain[0])
        loss_sploss.update(ad_loss.item())
        ...
        if (iter_num != 0 and (iter_num+1) % (iter_per_epoch) == 0):
            ...
            log.write(
                ...
                valid_args[0], valid_args[6], valid_args[3] * 100, valid_args[4] * 100,
                loss_classifier.avg, classifer_top1.avg,
                float(best_model_ACC), float(best_model_HTER * 100), float(best_model_AUC * 100), loss_sploss.avg, classifer_top1_domain.avg,
                time_to_str(timer() - start, 'min'),
                param_lr_tmp[0]))
            log.write('\n')
            time.sleep(0.01)
