...
from utils.utils import AverageMeter, accuracy, draw_roc, accuracy_eval
from utils.statistic import get_EER_states, get_HTER_at_thr, calculate, calculate_threshold
from sklearn.metrics import roc_auc_score, roc_curve, auc
from models.DGFAS_netvlad import DG_model_sharesp, DG_model
import shutil
...
def test(test_dataloader, model, threshold):
    ...
    with torch.no_grad():
        for iter, (input, target, videoID) in enumerate(test_dataloader):
            ...
            cls_out, _, _, _,_,_ = model(input, config.norm_flag)
    ...
    for key in prob_dict.keys():
        ...
        acc_valid2 = accuracy_eval(avg_single_video_output, avg_single_video_target, topk=(1,))

        valid_top1.update(acc_valid[0])
        #print(avg_single_video_label == avg_single_video_target)
        #print(acc_valid2)
        if avg_single_video_label == 1:
            valid_top1_real.append(acc_valid2[0])
        else:
            valid_top1_fake.append(acc_valid2[0])
    ...
    return [valid_top1.avg, cur_EER_valid, cur_HTER_valid, auc_score, ACC_threshold, threshold, np.array(valid_top1_real).mean(), np.array(valid_top1_fake).mean()]


def main():
    #net = DG_model_sharesp(config.model).cuda()
    #net = DG_model(config.model, num_cluster=32, dim=128, alpha=3.0, relu=True).cuda()
    net = DG_model_sharesp(config.model, num_cluster=32, dim=128, par_dim=(np.array([30, 2]) * 128).astype(int), alpha=3.0,
                     par_clu=np.array([30, 2]), nonorm=True, nog=False).cuda()
    ...
    print(config.tgt_data, 'Test acc_real: %5.4f' % (test_args[-2]))
    print(config.tgt_data, 'Test acc_fake: %5.4f' % (test_args[-1]))
