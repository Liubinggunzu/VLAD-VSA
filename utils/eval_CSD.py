...
def eval(valid_dataloader, model, norm_flag):
    ...
    with torch.no_grad():
        for iter, (input, target, videoID) in enumerate(valid_dataloader):
            ...
            cls_out, _, feature = model(input, norm_flag)
...
def eval6(valid_dataloader, model, norm_flag):
    criterion = nn.CrossEntropyLoss()
    valid_losses = AverageMeter()
    valid_top1 = AverageMeter()
    prob_dict = {}
    ...
    with torch.no_grad():
        for iter, (input, target, videoID) in enumerate(valid_dataloader):
            ...
            target = Variable(torch.from_numpy(np.array(target)).long()).cuda()
            # cls_out, _, feature = model(input, norm_flag)
            cls_out, _, _, _, _, _ = model(input, norm_flag)
            prob = F.softmax(cls_out, dim=1).cpu().data.numpy()[:, 1]
            label = target.cpu().data.numpy()
            videoID = videoID.cpu().data.numpy()
            for i in range(len(prob)):
...
def eval7(valid_dataloader, model, norm_flag):
    criterion = nn.CrossEntropyLoss()
    valid_losses = AverageMeter()
    valid_top1 = AverageMeter()
    prob_dict = {}
    ...
    with torch.no_grad():
        for iter, (input, target, videoID) in enumerate(valid_dataloader):
            ...
            target = Variable(torch.from_numpy(np.array(target)).long()).cuda()
            # cls_out, _, feature = model(input, norm_flag)
            cls_out, _, _, _, _, _, _ = model(input, norm_flag)
            prob = F.softmax(cls_out, dim=1).cpu().data.numpy()[:, 1]
            label = target.cpu().data.numpy()
            videoID = videoID.cpu().data.numpy()
            for i in range(len(prob)):
...
def eval8(valid_dataloader, model, norm_flag):
    criterion = nn.CrossEntropyLoss()
    valid_losses = AverageMeter()
    valid_top1 = AverageMeter()
    prob_dict = {}
    ...
    with torch.no_grad():
        for iter, (input, target, videoID) in enumerate(valid_dataloader):
            ...
            target = Variable(torch.from_numpy(np.array(target)).long()).cuda()
            # cls_out, _, feature = model(input, norm_flag)
            cls_out, _, _, _, _, _, _, _ = model(input, norm_flag)
            prob = F.softmax(cls_out, dim=1).cpu().data.numpy()[:, 1]
            label = target.cpu().data.numpy()
            videoID = videoID.cpu().data.numpy()
            for i in range(len(prob)):
...
def eval9(valid_dataloader, model, norm_flag):
    criterion = nn.CrossEntropyLoss()
    valid_losses = AverageMeter()
    valid_top1 = AverageMeter()
    prob_dict = {}
    ...
    with torch.no_grad():
        for iter, (input, target, videoID) in enumerate(valid_dataloader):
            ...
            target = Variable(torch.from_numpy(np.array(target)).long()).cuda()
            # cls_out, _, feature = model(input, norm_flag)
            cls_out, _, _, _, _, _, _, _, _ = model(input, norm_flag)
            prob = F.softmax(cls_out, dim=1).cpu().data.numpy()[:, 1]
            label = target.cpu().data.numpy()
            videoID = videoID.cpu().data.numpy()
            for i in range(len(prob)):
...
def eval5(valid_dataloader, model, norm_flag):
    criterion = nn.CrossEntropyLoss()
    valid_losses = AverageMeter()
    valid_top1 = AverageMeter()
    prob_dict = {}
    ...
    with torch.no_grad():
        for iter, (input, target, videoID) in enumerate(valid_dataloader):
            ...
            target = Variable(torch.from_numpy(np.array(target)).long()).cuda()
            # cls_out, _, feature = model(input, norm_flag)
            cls_out, _, _, _, _ = model(input, norm_flag)
            prob = F.softmax(cls_out, dim=1).cpu().data.numpy()[:, 1]
            label = target.cpu().data.numpy()
            videoID = videoID.cpu().data.numpy()
            for i in range(len(prob)):
