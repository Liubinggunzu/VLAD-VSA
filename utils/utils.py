...
def sample_frames(flag, num_frames, dataset_name):
    ...
    #root_path = '/home1/share/anti-spoofing/' + dataset_name
    #if not os.path.exists(root_path):
    root_path = '/home1/wangjiong/dataset/FAS/' + dataset_name
    ...
    single_video_label_ID = 0
    for i in range(length):
        photo_path = all_label_json[i]['photo_path']
        photo_label = all_label_json[i]['photo_label']
        # if exist()
        photo_label_ID = all_label_json[i]['photo_label_ID']
        # print(all_label_json[i])
        ...
        if (i == length - 1):
            ...
            single_video_label_ID = photo_label_ID
        ...
        if (frame_prefix != saved_frame_prefix or i == length - 1):
            ...
            for j in range(num_frames):
                dict = {}
                #if len(single_video_frame_list) < 100:
                    #print(len(single_video_frame_list), saved_frame_prefix)
                dict['photo_path'] = saved_frame_prefix + '/' + str(
                    single_video_frame_list[6 + j * frame_interval]) + '.jpg'
                dict['photo_label'] = single_video_label
                dict['photo_label_ID'] = photo_label_ID
...
class AverageMeter_real(object):
    """Computes and stores the average and current value"""
    def __init__(self):
        self.reset()
...
class AverageMeter_fake(object):
    """Computes and stores the average and current value"""
    def __init__(self):
        self.reset()
...
def accuracy_eval(output, target, topk=(1,)):
    with torch.no_grad():
        maxk = max(topk)
        _, pred = output.topk(maxk, 1, True, True)
        pred = pred.t()
...
def to_categorical_ml(y, num_classes=None, dtype='float32'):
    #y = np.array(y, dtype='int')
    #y = y.ravel()
    # categorical = np.zeros((y.shape[0], num_classes), dtype=dtype)
    categorical = torch.zeros(y.shape[0], num_classes).to(y.device)
